...
void LinkedList_Free(LinkedList *list,
  ...
  // free the payloads and nodes one-by-one using a loop
  while (curr != NULL) {
...
bool LinkedList_Pop(LinkedList *list, LLPayload_t *payload_ptr) {
  ...
  if (list->num_elements == 1) {
    // degenerate case: only one element in the list
...
void LinkedList_Append(LinkedList *list, LLPayload_t payload) {
  ...
  // pretty much like push, instead we add to the end
  if (list->num_elements == 0) {
    Verify333(list->head == NULL);
    Verify333(list->tail == NULL);
    ln->next = ln->prev = NULL;
...
bool LLIterator_Remove(LLIterator *iter,
  ...
  // when only 1 element in the list, remove and return false
  ...
  if (iter->node == iter->list->head) {
    // when we are removing the head, using LLIterator_Pop
    LLPayload_t payload;
    LinkedList_Pop(iter->list, &payload);
    payload_free_function(payload);
    iter->node = iter->list->head;
  } else if (iter->node == iter->list->tail) {
    // when we are removing the tail, using LLIterator_Slice
    LLPayload_t payload;
...
bool LinkedList_Slice(LinkedList *list, LLPayload_t *payload_ptr) {
  ...
  // pretty much like pop but instead we are removing from the tail
  if (list->num_elements == 0) {
    return false;
  }
