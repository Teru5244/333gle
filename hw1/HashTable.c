...
// Helper function that helps to find if a key is within a chain given
// an iterator of the chain, the key to be found, and a return value.
// Will return the keyvalue found to the return value.
static bool Chain_Find(LLIterator* iter, HTKey_t key, HTKeyValue_t** returnVal);
...
static bool Chain_Find(LLIterator* iter, HTKey_t key, HTKeyValue_t** returnVal) {
  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t*) returnVal);
    if ((*returnVal)->key == key) {
      return true;
    }
...
bool HashTable_Insert(HashTable *table,
  ...
  // Creating copies to avoid memory leak
  HTKeyValue_t* newCopy = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));
  HTKeyValue_t* oldCopy;
  Verify333(newCopy != NULL);

  // copy to the new allocated keyvalue
  newCopy->key = newkeyvalue.key;
  newCopy->value = newkeyvalue.value;

  // insert directly if no elements in the chain
  ...
  if (Chain_Find(iter, newkeyvalue.key, &oldCopy)) {
    LinkedList_Push(chain, (LLPayload_t) newCopy); // if found

    oldkeyvalue->key = oldCopy->key;
    oldkeyvalue->value = oldCopy->value;

    // Remove the original value
    LLIterator_Remove(iter, &LLNoOpFree);

    // free the old payload after using it
    free(oldCopy);
    LLIterator_Free(iter);

    return true;
  } else {
    LinkedList_Push(chain, (LLPayload_t) newCopy); // if not found, just push directly
    LLIterator_Free(iter);

    // and add 1 to the element count
    table->num_elements++;
...
bool HashTable_Find(HashTable *table,
  ...
  // pretty much the same schedules as insert, except that
  // we do not need to replace or push anything new

  int bucket;
  LinkedList *chain;

  // Calculate which bucket and chain we're looking up into.
  ...
  // the payload we found for copying
  HTKeyValue_t* copy;
  LLIterator* iter = LLIterator_Allocate(chain);

  if (Chain_Find(iter, key, &copy)) {
    keyvalue->key = copy->key; // if found, copy the value
    keyvalue->value = copy->value;
    LLIterator_Free(iter);

    return true;
  } else {
    LLIterator_Free(iter); // if not found
...
bool HashTable_Remove(HashTable *table,
  ...
  // pretty much the same steps as insert and find
  ...
  if (Chain_Find(iter, key, &copy)) {
    keyvalue->key = copy->key; // if found, delete and copy
    keyvalue->value = copy->value;

    free(copy);
    table->num_elements--; // remember to decrement the num_elements
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // if the hash table is empty just return false
  if (HashTable_NumElements(iter->ht) == 0) {
    return false;
  }

  if (!LLIterator_IsValid(iter->bucket_it)) {
    // if we pass the end of a chain, that means we have finished
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  if (HTIterator_IsValid(iter)) {
    // if valid, we check if we can advance in the chain
    bool advance = LLIterator_Next(iter->bucket_it);

    if (!advance) {
      // if we can't advance, then search for next chain that has elements
      int i;
      ...
      if (i < iter->ht->num_buckets) {
        ...
        Verify333(i < iter->ht->num_buckets);
        // free the iterator first and allocate a new one
        LLIterator_Free(iter->bucket_it);
        iter->bucket_it = LLIterator_Allocate(iter->ht->buckets[iter->bucket_idx]);

        return true;
      } else {
        // next chain not found
        return false;
      }
    } else {
      // if we advance in the chain, just return true
      return true;
    }
  } else {
    // not valid
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  if (HTIterator_IsValid(iter)) {
    // if valid, copy and return
    HTKeyValue_t* copy;

    LLIterator_Get(iter->bucket_it, (LLPayload_t*) &copy);
