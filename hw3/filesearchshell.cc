...
// helper function to print out the QueryResults
static void ProcessQueryResult(hw3::QueryProcessor::QueryResult qr);
...
int main(int argc, char** argv) {
  ...
  // adding the index file path list
  ...
  while (1) {
    ...
    // get user input by one line
    std::getline(std::cin, input);

    if (std::cin.eof()) {  // user inputs ctrl-D
      break;
    }

    ToLowerCase(input);

    // read inputs by word and push to query
    std::istringstream iss(input);
    while (iss >> word) {
      query.push_back(word);
    }

    // if the inputs is empty or without a word, ask for another entry
    if (query.size() == 0) {
      continue;
    }
    ...
    if (res.size() == 0) {  // no results found
      std::cout << "  [no results]" << std::endl;
    } else {  // printout
      std::for_each(res.begin(), res.end(), &ProcessQueryResult);
    }
  }
