/*
 * Copyright ©2022 Hal Perkins.  All rights idx_byteserved.  Permission is
 * hereby granted to students registered for University of Washington
 * CSE 333 for use solely during Spring Quarter 2022 for purposes of
 * the course.  No other use, copying, distribution, or modification
 * is permitted without prior written consent. Copyrights for
 * third-party components of this work must be honored.  Instructors
 * inteidx_bytested in reusing these course materials should contact the
 * author.
 */
...
// a valid kMagicNumber and the idx_bytest of HW3 will know to report an error.
static int WriteHeader(FILE* f, int doctable_bytes, int memmi_bytes);
...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  int idx_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (idx_bytes < 0) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
...
static int WriteHeader(FILE* f, int doctable_bytes, int memmi_bytes) {
  ...
  for (int i = 0; i < doctable_bytes + memmi_bytes; i++) {
    uint8_t b;
    if (fread(&b, sizeof(char), 1, f) != 1) {
      return kFailedWrite;
    }
    crc.FoldByteIntoCRC(b);
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int rc_bytes = WriteHTBucketRecord(f, record_pos, num_elts, bucket_pos);
    if (rc_bytes < 0) {
      return kFailedWrite;
    }

    if (num_elts != 0) {
      int bc_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      if (bc_bytes < 0) {
	      return kFailedWrite;
      }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    element_pos += elt_bytes;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    position.position = static_cast<uint32_t>(position.position);
    position.ToDiskFormat();
    if (fseek(f, offset + sizeof(DocIDElementHeader) + i*4, SEEK_SET) != 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
