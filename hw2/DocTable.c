...
void DocTable_Free(DocTable* table) {
  ...
  HashTable_Free(table->id_to_name, &free); // using free() function to free the payload
...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  name_key = FNVHash64((unsigned char*) doc_name, strlen(doc_name));
  if (HashTable_Find(table->name_to_id, name_key, &old_kv)) {
    return *((DocID_t*) old_kv.value);
  }

  // if no existing, then allocate and copy
