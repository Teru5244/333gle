  ...
  while (left_to_read > 0) {
    ...
    if (result == -1) { // error
      if (errno != EINTR && errno != EAGAIN) {
        // not EINTR or EAGAIN, exit
        free(buf);
        return NULL;
      }
      // hit EINTR or EAGAIN, try again
      continue;
    } else if (result == 0) {
      // EOF reached
      break;
    } else {
      // add num_read and decrease left_to_read
      num_read += result;
      left_to_read -= result;
    }
  }
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (1) {
    if (*cur_ptr == '\0') {
      ...
      // is a valid character of a word, then transfer to lower case
      *cur_ptr = tolower(*cur_ptr);
    } else {
      // reaches boundary character
      *cur_ptr = '\0';
      // if we have a valid char before cur_ptr, then he hit the end of a word
      if (cur_ptr != content && isalpha(*(cur_ptr - 1)) != 0) {
        AddWordPosition(tab, word_start, word_start - content);
      }
      // if we have a valid char after cur_ptr, then we set the word_start
      if (isalpha(*(cur_ptr + 1)) != 0) {
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // create a new WordPosition struct
    wp = (WordPositions*) malloc(sizeof(WordPositions));
    Verify333(wp != NULL);

    // allocate new word and copy the old word to it
    new_word = (char*) malloc(sizeof(char) * strlen(word) + 1);
    Verify333(new_word != NULL);
    strcpy(new_word, word);

    // add word and allocate new linked list to wp
    wp->word = new_word;
