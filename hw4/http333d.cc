...
static void GetPortAndPath(int argc,
  ...
  *port = atoi(argv[1]);
  ...
  if (stat(argv[2], &dir_stat) == -1) {
    cerr << "Path is not readable." << endl;
    Usage(argv[0]);
  }

  // check if path not a directory
  ...
  *path = argv[2];
  ...
  for (int i = 3; i < argc; i++) {
    if (stat(argv[i], &dir_stat) == -1) {
      cerr << "Indices path is not readabable." << endl;
      Usage(argv[0]);
    }

    // check if path not a directory
    if (!S_ISREG(dir_stat.st_mode)) {
      cerr << "Indices files are not readable." << endl;
      Usage(argv[0]);
    }
  ...
  // check if indices contains readable files
  if (indices->size() == 0) {
