...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char* res = realpath(root_dir.c_str(), root_buf);
  if (res == NULL) {
    return false;
  }

  res = realpath(test_file.c_str(), path_buf);
  if (res == NULL) {
    return false;
  }
  ...
  return strncmp(root_buf, path_buf, strlen(root_buf)) == 0;  // You may want to change this.
}
